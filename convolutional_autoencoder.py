...
class ConvolutionalAutoencoder(object):
    ...
    def reconstruct(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(batch_size):
                im = np.concatenate((org[i, :, :, :].reshape(28, 28), recon[i, :, :, :].reshape(28, 28)), axis=0)
    ...
    def conv_layer(self,
        ...
        weights = tf.Variable(tf.truncated_normal(weights_shape, stddev=weights_init_stddev))
        bias = tf.Variable(tf.constant(bias_init_value, shape=[num_output_channels]))  # 1 bias for each output channel
    ...
    def fully_connected_layer(self,
        ...
        weights = tf.Variable(tf.truncated_normal(weights_shape, stddev=weights_init_stddev))
        bias = tf.Variable(tf.constant(bias_init_value, shape=[num_neurons]))
